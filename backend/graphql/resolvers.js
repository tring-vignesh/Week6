...
const resolvers = {
    Query: {
        ...
        getUserPersonas: async (parent, args, context) => {
            ...
            // const { rows } = await pool.query("SELECT * FROM personas WHERE user_id = $1", [user.userId]);
    ...
    Mutation:
    {
        ...
        addPersona: async (parent, { input, persona_image }, context) => {
          const { name, quote, description, motivation, pain_points, jobs_needs, activites } = input;
            ...
            let imageUrl = null;
            if (persona_image) {
              const { createReadStream, filename } = await persona_image;
            ...
            const { rows } = await pool.query(
              ...
              [name, quote, description, motivation, pain_points, jobs_needs, activites, imageUrl, "1"]
            );
            return rows[0];
          },
          updatePersona: async (parent,  { id, input }, context) => {
            ...
            const { rows } = await pool.query("SELECT * FROM personas WHERE id = $1 AND user_id = $2", [id, "1"]);
            if (rows.length === 0) throw new Error("Persona not found or unauthorized");
            ...
            const updatedPersona = await pool.query(
              ...
              [name, quote, description, motivation, pain_points, jobs_needs, activites, persona_image, id]
            );
